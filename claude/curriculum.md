...
_Section 3 direction, pending confirmation (2026-07-14)_: Not yet a committed rewrite of Sections 3-5 below — a tentative direction to make real once Section 3 is actually done, so the reasoning isn't lost if it sticks. Context: `massspecgym` got pinned to the v1.5 commit (`f259fe3`, the unreleased `feat/massspecgym-v1.5` merge — never cut to PyPI) so that Section 2's environment can genuinely reproduce and score against the MassSpecGym v1.5 "in the Wild" pitfalls paper (`papers/MassSpecGym v1.5 common pitfalls.pdf`), and Section 2 was re-verified working against it. Reading that paper's Table 3 retrieval leaderboard surfaced two goals, both more specific than the original "tour DreaMS" / "tour MIST" framing: (A) the data-distribution/shortcut-learning problem from §5.2 — spectrum-blind baselines (`PubChem default ranking`, `ranking by num. of chiral atoms`) exploit the distributional mismatch between MassSpecGym's ground-truth molecules and PubChem-derived decoys, scoring suspiciously well without reading the spectrum at all; and (B) getting a working grip on the current retrieval-model ecosystem, scoped down — given the time budget — to just the two backbone encoders reused across nearly every strong Table 3 entry, MIST and DreaMS, rather than a survey of everything (JESTR, MVP, FLARE, DiffMS, ICEBERG, FRIGID stay as literature-awareness, not build targets, unless one specifically pulls me in later).

If this sticks: Section 3 tackles Goal A first — reproduce the two §5.2 spectrum-blind baselines as `RetrievalMassSpecGymModel` subclasses (same contract as Section 2; no DreaMS/MIST needed, since these baselines are deliberately spectrum-blind) as a checkpoint that the harness scores correctly, then use that grounding to explore reframing the problem (e.g. property-matched decoys, per Gupta & Skinnider, cited in the paper as concurrent work). Sections 4-5 then cover MIST and DreaMS specifically as encoder backbones in service of Goal B, not general tours. This is closer to my actual PhD interest (coverage bias / shortcut learning) than the original tour framing — decide for real whether to formally renumber Sections 3-5 once Section 3 is complete.
